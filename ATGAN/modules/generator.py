...
class Generator(nn.Module):
    ...
    def forward(self,ir_img,vi_img):
        ...
        ir_layer3 = self.leaky_relu(self.ir_bn3(self.ir_conv3(torch.cat([ir_layer2, ir_layer1,ir_img], dim=1))))
        ...
        x_final = self.gfusion(ir_img, ir_layer1,
                               ir_layer2,
                               ir_layer3,vi_img, vi_layer1,
                               vi_layer2, vi_layer3)
...
class GFusion(nn.Module):
    def __init__(self,input_channels,dense_channels):
        ...
        self.sn_conv1=Conv2d(34, 8, kernel_size=3, stride=1)
        self.bn1 = nn.BatchNorm2d(8, momentum=0.9, eps=1e-5)
        self.sn_conv2 = Conv2d(42, 1, kernel_size=1, stride=1)
    ...
    def forward(self,ir_img, ir_layer1,
                               ir_layer2,
                               ir_layer3,vi_img, vi_layer1,
                               vi_layer2, vi_layer3):

        ir_features=torch.cat([ir_img, ir_layer1,
                               ir_layer2,
                               ir_layer3], dim=1)
        vi_features = torch.cat((vi_img, vi_layer1,
                               vi_layer2, vi_layer3),dim=1)
        x=torch.cat([ir_features*1.5,vi_features],dim=1)
