...
class Matchformer_SEA_lite(nn.Module):
    def __init__(self, img_size=224, in_chans=1, embed_dims=[64, 192, 192, 256], num_heads=[1, 2, 4, 8],
        ...
        self.sem = nn.Sequential(AttentionBlock(img_size=img_size // 4, patch_size=3, num_heads=num_heads[1],
                                              mlp_ratios=4, in_chans=embed_dims[0],
                                              embed_dims=128, stride=2, sr_ratios=sr_ratios[1], depths=1,
                                              cross=[False]),JConv(128,128),JConv(128,128),JConv(128,128),JConv(128,128),JConv(128,64),JConv(64,1))
...
class JConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, path_dropout=0.,
                 drop=0., mlp_ratio=3, isfirst=False):
...
class MLP(nn.Module):
    def __init__(self, in_features, mlp_ratio=None, drop=0., bias=True):
        super().__init__()
        self.conv1 = nn.Conv2d(in_features, in_features * 3, kernel_size=1, bias=bias)
        self.act = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv2d(in_features * 3, in_features, kernel_size=1, bias=bias)
        self.drop = nn.Dropout(drop)
