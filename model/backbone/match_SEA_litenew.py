...
def mask_grid(x,y):
    xs = torch.linspace(0, x - 1, x)
    ys = torch.linspace(0, y - 1, y)
    xs = xs / (x - 1)
    ys = ys / (y - 1)
    grid = torch.stack(torch.meshgrid([xs, ys]), dim=-1).unsqueeze(0).repeat(2, 1, 1, 1).cuda()
    grid = rearrange(grid, 'n h w c -> n c h w')
    return grid


class Matchformer_SEA_lite(nn.Module):
    def __init__(self, img_size=224, in_chans=1, embed_dims=[64, 192, 192, 256], num_heads=[1, 2, 4, 8],
                 sr_ratios=[8, 4, 2, 1]
                 , stage1_cross=[False], stage2_cross=[False, True, False],
                 stage3_cross=[False, True, False, True], stage4_cross=[False, True, False, True]):
        super().__init__()
        # Attention
        self.AttentionBlock1 = AttentionBlock(img_size=img_size // 2, patch_size=7, num_heads=num_heads[0],
                                              mlp_ratios=4, in_chans=in_chans,
                                              embed_dims=embed_dims[0], stride=4, sr_ratios=sr_ratios[0], depths=1,
                                              cross=stage1_cross)
        self.AttentionBlock2 = AttentionBlock(img_size=img_size // 4, patch_size=3, num_heads=num_heads[1],
                                              mlp_ratios=4, in_chans=embed_dims[0] + 2,
        ...
        self.AttentionBlock3 = AttentionBlock(img_size=img_size // 16, patch_size=3, num_heads=num_heads[2],
                                              mlp_ratios=4, in_chans=embed_dims[1] + 2,
                                              embed_dims=embed_dims[2], stride=2, sr_ratios=sr_ratios[2], depths=4,
                                              cross=stage3_cross)
        self.AttentionBlock4 = AttentionBlock(img_size=img_size // 32, patch_size=3, num_heads=num_heads[3],
                                              mlp_ratios=4, in_chans=embed_dims[2] + 2,
                                              embed_dims=embed_dims[3], stride=2, sr_ratios=sr_ratios[3], depths=4,
                                              cross=stage4_cross)

        # FPN
        self.layer4_outconv = conv1x1(embed_dims[3] + 2, embed_dims[3] + 2)
        self.layer3_outconv = conv1x1(embed_dims[2] + 2, embed_dims[3] + 2)
        self.layer3_outconv2 = nn.Sequential(
            conv3x3(embed_dims[3] + 2, embed_dims[3] + 2),
            nn.BatchNorm2d(embed_dims[3] + 2),
            nn.LeakyReLU(),
            conv3x3(embed_dims[3] + 2, embed_dims[2] + 2),
        )
        self.layer2_outconv = conv1x1(embed_dims[1] + 2, embed_dims[2] + 2)
        self.layer2_outconv2 = nn.Sequential(
            conv3x3(embed_dims[2] + 2, embed_dims[2] + 2),
            nn.BatchNorm2d(embed_dims[2] + 2),
            nn.LeakyReLU(),
            conv3x3(embed_dims[2] + 2, embed_dims[1] + 2),
        )
        ...
        self.grid1 = mask_grid(int(h / 4), int(w / 4))
        self.grid2 = mask_grid(60, int(w / 8))
        self.grid3 = mask_grid(30, int(w / 16))
        self.grid4 = mask_grid(15, int(w / 32))
        # self.grid3 = mask_grid(15, int(w / 16))
        # self.grid4 = mask_grid(8, int(w / 32))
        # self.grid3 = mask_grid(30, int(w / 16))
        # self.grid4 = mask_grid(15, int(w / 32))
